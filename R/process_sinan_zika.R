...
process_sinan_zika <- function(data, municipality_data = TRUE){
  ...
  if ("ID_PAIS" %in% variables_names) {
    data$ID_PAIS <- dplyr::left_join(data, pgsscdata::paisnet, by = c("ID_PAIS" = "ID_PAIS"))$NM_PAIS
  }

  # ID_OCUPA_N
  if ("ID_OCUPA_N" %in% variables_names) {
    data$ID_OCUPA_N <- factor(dplyr::left_join(data, pgsscdata::tabCBO, by = c("ID_OCUPA_N" = "cod"))$nome)
  }
  ...
  if ("COPAISINF" %in% variables_names) {
    data$COPAISINF <- dplyr::left_join(data, pgsscdata::paisnet, by = c("COPAISINF" = "COPAISINF"))$NM_PAIS
  }
