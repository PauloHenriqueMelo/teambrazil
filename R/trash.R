...
trash <- function(data) {
  ...
  variables_to_keep <- c(
    ...
    "VAL_UTI", "US_TOT", "DT_INTER", "DT_SAIDA", "DIAG_PRINC", "DIAG_SECUN", "DIAGSEC1", "TPDISEC1",
    "DIAGSEC2", "TPDISEC2",
    "DIAGSEC3", "TPDISEC3",
    "DIAGSEC4", "TPDISEC4",
    "DIAGSEC5", "TPDISEC5",
  ...
  if ("Year_Competency" %in% names(data)) {
    # Convert Year_Competency to numeric if it's a character
    data$Year_Competency <- suppressWarnings(as.numeric(data$Year_Competency))

    # Create SecondaryDiagnosis based on the condition
    data$SecondaryDiagnosis <- NA_character_  # Default to NA

    if ("DIAG_SECUN" %in% names(data) && "DIAGSEC1" %in% names(data)) {
      data$SecondaryDiagnosis <- ifelse(
        data$Year_Competency <= 2014,
        as.character(data$DIAG_SECUN),
        as.character(data$DIAGSEC1)
      )
      data <- data %>% dplyr::select(-DIAG_SECUN, -DIAGSEC1)
    } else if ("DIAG_SECUN" %in% names(data)) {
      data$SecondaryDiagnosis <- ifelse(
        data$Year_Competency <= 2014,
        as.character(data$DIAG_SECUN),
        NA_character_
    ...
    } else if ("DIAGSEC1" %in% names(data)) {
      data$SecondaryDiagnosis <- ifelse(
        data$Year_Competency > 2014,
        as.character(data$DIAGSEC1),
        NA_character_
      )
      data <- data %>% dplyr::select(-DIAGSEC1)
    }
  } else {
    data$SecondaryDiagnosis <- NA_character_  # If Year_Competency is missing
  }
